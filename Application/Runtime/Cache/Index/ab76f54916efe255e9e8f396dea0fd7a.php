...
<body class="gray-bg">
						...
						<div class="row">
							<div class="col-sm-3">
								<table class="table table-hover margin bottom">
									...
									<tbody>
										<?php if(is_array($userx)): $i = 0; $__LIST__ = $userx;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr style="cursor:pointer" id="<?php echo $vo['id'];?>" class="staffdata">
											<td class="text-center"><?php echo $vo['name'];?></td>
											<td><?php echo $vo['role_name'];?></td> 
											<td class="text-right"><?php echo $vo['waybill_num'];?></td>                                            
											<td class="text-right"><?php echo $vo['info_online'];?></td>
    ...
    <script>
        $(document).ready(function () {
			var markerArr = <?php echo $userx_json;?>;
			...
			for (var i = 0; i <len; i++) 
			{
				...
				//var myIcon = new BMap.Icon("/Public"+icon, new BMap.Size(100, 100));
			...
			$(".staffdata").click(function(){
				...
				var id=$(this).attr('id');
				var reponseurl='<?php echo U("index/user/get_staff_pos");?>';
				var postdata={id:id};
		...
		function baidu_map(longitude,latitude,icon,name,role_name,orders){
			...
			var myIcon = new BMap.Icon("/Public"+icon, new BMap.Size(50, 50));
