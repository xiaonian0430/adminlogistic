...
<!-- Modal -->
    ...
    <div class="modal-dialog">
        <div class="modal-content1  modal-content animated fadeIn">
            <div class="modal-header">
                ...
                <h6 class="modal-title" style="color:black;color:white">添加物流公司</h6>
            </div>
	...
	var mydata =<?php echo $waycompany;?>;
    /*
	var mydata = [
   {
        id: "1",
    ...
    $("#table_list_2").jqGrid({
        ...
        rowNum: 20,
        /*rowList: [10, 20, 30],*/
        colNames: ["编号", "物流公司名称", "所在区域","所在城市", "详细地址", "负责人", "联系电话","操作"],
        colModel: [
        {
            name: "id",
            index: "id",
			...
			$(this).parent().trigger('click');
				...
				if (gr != null){
					...
					$("#sData").click(function(){
						var editurl='<?php echo U("index/info/wlgs_edit");?>';
			...
			$(this).parent().trigger('click');
				...
				if (gr != null){
					...
					$("#dData").click(function(){
						var delurl='<?php echo U("index/info/wlgs_delete");?>';
		...
		//editurl:'<?php echo U("index/info/wlgs_edit");?>',
    });
    ...
    $(".glyphicon-plus").click(function(){
		...
		$("#sData").click(function(){
			var editurl='<?php echo U("index/info/wlgs_add");?>';
			var name=$.trim($(".FormGrid #name").val());
			var area=$.trim($(".FormGrid #area").find("option:selected").text());
			var subarea=$.trim($(".FormGrid #subarea").find("option:selected").text());
			var address=$.trim($(".FormGrid #address").val());
			var leader=$.trim($(".FormGrid #leader").val());
			var tel=$.trim($(".FormGrid #tel").val());
			var postdata={name:name,area:area,subarea:subarea,address:address,leader:leader,tel:tel};
			$.post(editurl,postdata,function(data){
				var code=data.code;
				var msg=data.msg;
				//swal(msg, "", "error");
